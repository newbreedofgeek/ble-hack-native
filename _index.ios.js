...
var measurementCharacteristicBP = null;
...
class BleHackNative extends Component {
  constructor(props) {
    ...
    this.readPeripheralDataBP = this.readPeripheralDataBP.bind(this);
  ...
  componentDidMount() {
    ...
    noble.on('discover', function(peripheral) {
      ...
      if (peripheral.advertisement.localName == Devices.localNameWeight) {
        ...
        peripheral.connect(function(err) {
          peripheral.discoverServices([Devices.serviceUuidBP], function(err, services) {
            services.forEach(function(service) {
              console.log('found our target BP service:', service.uuid);
              service.discoverCharacteristics([], function(err, characteristics) {

                characteristics.forEach(function(characteristic) {
                  if (Devices.measurementCharacteristicUuidBP == characteristic.uuid.toLowerCase()) {
                    console.log('found our target BP characteristic:', characteristic.uuid);
                    measurementCharacteristicBP = characteristic;

                    this.readPeripheralDataBP();
                  }
                }.bind(this))
              }.bind(this))
            }.bind(this))
  ...
  readPeripheralDataBP() {
    ...
    measurementCharacteristicBP.on('data', function(data, isNotification) {
      ...
      var adjustedReading = "systolic = " + rawData['1'] + " : diastolic = " + rawData['3'] + " : pulse = " + rawData['7'];
      ...
      if (this.state.groupMode == 1) {
        this.groupModeGetActiveUser(adjustedReading, 2);
      }
      else {
        this.saveAndReset(adjustedReading, 2);
      }
  ...
  render() {
    ...
    var deviceBPStyle = styles.deviceIdle;
    ...
    if (this.state.deviceBPState == 2) {
      deviceBPStyle = styles.deviceWorking;
    }
    else if (this.state.deviceBPState == 3) {
      deviceBPStyle = styles.deviceReceived;
    }
      ...
      <View style={styles.holder}>
        ...
        <View style={styles.container}>
          ...
          <View style={styles.rightOpts}>
            ...
            <View>
              <Text style={styles.instructions}>BP</Text>
              <View style={deviceBPStyle}></View>
            </View>
          </View>
        </View>
      </View>
